"""extract_characters agent:单批人物增量抽取(1 次 LLM 调用)。

给定一批章节正文 + 已知人物表,LLM 返回本批的人物 delta;合并/累积由调用方
(workflow)负责。
...
``CharacterExtraction``)全部住在本 agent 内。
"""

from agents.extract_characters.logic import SYSTEM_PROMPT, extract_for_batch
